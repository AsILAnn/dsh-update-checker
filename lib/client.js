		...
		*/
		...
		function UpdatePage() {
			...
			react.useEffect(() => {
				...
				console.info("[dsh-update-checker] 恢复：data=" + (saved.data ? "yes" : "NO") + " status=" + (saved.status && saved.status.code || "NO") + " lastCheckAt=" + t + " build=0.7.5+20260913-1914");
			}, []);
			react.useEffect(() => {
				let alive = true;
			...
			const buildStamp = String("0.7.5+20260913-1914");
			const shell = {
				fontFamily: "inherit",
				fontSize: 12,
				lineHeight: 1.6,
			...
			const lastCheckHint = lastCheck ? react.createElement("span", { style: {
				marginLeft: "auto",
				fontSize: 11,
				color: "var(--dsw-alias-label-tertiary,#a2a4a6)"
			} }, "上次检查：" + agoText(lastCheck)) : null;
			...
			return react.createElement("div", { style: shell }, styleTag, title, desc, busy ? react.createElement(CheckingCard) : react.createElement("div", {
				...
				disabled: updating,
				onClick: doUpdate
			}, updating ? react.createElement(Spinner, { size: 13 }) : null, updating ? "更新中…" : "立即更新") : null, lastCheckHint), updating ? react.createElement(UpdatingCard, {
