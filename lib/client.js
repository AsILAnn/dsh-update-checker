		...
		*/
		...
		function UpdatePage() {
			...
			react.useEffect(() => {
				...
				console.info("[dsh-update-checker] 恢复：data=" + (saved.data ? "yes" : "NO") + " status=" + (saved.status && saved.status.code || "NO") + " lastCheckAt=" + t + " build=0.7.7+20260914-2026");
			}, []);
			react.useEffect(() => {
			...
			const buildStamp = String("0.7.7+20260914-2026");
			const shell = {
				fontFamily: "inherit",
				fontSize: 12,
				lineHeight: 1.6,
			...
			if (entry) {
				...
				return react.createElement("div", { style: shell }, styleTag, title, desc, react.createElement("div", {
					...
					gap: 8,
					marginTop: 24
				} }, checkBtn, hasResult ? react.createElement("button", {
