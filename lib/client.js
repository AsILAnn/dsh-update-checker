		...
		*/
		...
		function UpdatePage() {
			...
			react.useEffect(() => {
				...
				console.info("[dsh-update-checker] 恢复：data=" + (saved.data ? "yes" : "NO") + " status=" + (saved.status && saved.status.code || "NO") + " lastCheckAt=" + t + " build=0.7.8+20260914-2037");
			}, []);
			react.useEffect(() => {
				if (cacheInfo || infoInFlight) return;
				infoInFlight = true;
				let alive = true;
				fetch(API + "/info", { headers: { "content-type": "application/json" } }).then((r) => r.json()).then((d) => {
					infoInFlight = false;
			...
			const buildStamp = String("0.7.8+20260914-2037");
			const shell = {
				fontFamily: "inherit",
				fontSize: 12,
				lineHeight: 1.6,
			...
			if (entry) {
				...
				const note = checked ? (status ? status.label : "已有结果") + (lastCheck ? " · 检查于 " + clockText(lastCheck) + "（" + agoText(lastCheck) + "）" : "") : "尚未检查 · 点下方按钮对比官方最新版本";
				return react.createElement("div", { style: shell }, styleTag, title, desc, react.createElement("div", {
				...
				}, "本机版本"), react.createElement("div", { style: {
					fontSize: 28,
					fontWeight: 500,
					lineHeight: 1.15,
					...
					fontFamily: MONO,
					fontVariantNumeric: "tabular-nums",
					color: shown ? void 0 : "var(--dsw-alias-label-tertiary,#a2a4a6)"
				} }, shown || "未获取"), react.createElement("div", {
					className: outdated ? void 0 : "dsh-upd-ink2",
					style: {
						fontSize: 12,
						marginTop: 8,
						color: outdated ? BAD : void 0
					},
					title: info && info.installDir ? "安装目录：" + info.installDir : void 0
				}, note)), react.createElement("div", { style: { marginTop: 24 } }, checkBtn));
			}
